...
import { PreferencesService } from 'src/app/core/services/preference.service';
import { Club } from 'src/app/core/services/preference.service';
...
export class PostViewComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (this.post.image_url && !this.post.is_official) {
      //console.log('Post image URL not official:', this.post.image_url);
      this.postSafeUrl = await this.imageDownloadService.loadPostImage(this.post.image_url);
      //console.log('Post safe URL:', this.postSafeUrl);
    }
    ...
    const preferences = await this.preferenceService.getPreferences(this.post.user_id);
    const favoriteClubPreference = preferences.find(preference => preference.favorite_club);
    if (favoriteClubPreference && favoriteClubPreference.club_id) {
      this.favoriteClub = await this.preferenceService.getClubByClubId(parseInt(favoriteClubPreference.club_id));
    }
  }
  ...
  async getUsernameRetweetedById(id: string): Promise<any> {
    this.userService.getUsernameByID(id).then(async username => {
      this.usernameRetweeted = username;
      await this.getRetweetedUserFavoriteClub(id);
    }).catch(error => {
      console.error('Could not fetch username', error);
    });
  }

  async getRetweetedUserFavoriteClub(id: string): Promise<any> {
    try {
      const preferences = await this.preferenceService.getPreferences(id);
      const favoriteClubPreference = preferences.find(pref => pref.favorite_club);
      if (favoriteClubPreference && favoriteClubPreference.club_id) {
        this.clubRetweeted = await this.preferenceService.getClubByClubId(parseInt(favoriteClubPreference.club_id));
      }
    } catch (error) {
      console.error('Could not fetch favorite club', error);
    }
  }
