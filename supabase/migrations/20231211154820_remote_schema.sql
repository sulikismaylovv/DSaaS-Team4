alter table "public"."usersinfriendsleague" drop constraint "usersinfriendsleague_userid_fkey";

alter table "public"."usersinfriendsleague"
  add constraint "usersinfriendsleague_userid_fkey" FOREIGN KEY (userid) REFERENCES users (id) ON UPDATE CASCADE ON DELETE CASCADE not valid;

alter table "public"."usersinfriendsleague" validate constraint "usersinfriendsleague_userid_fkey";

set
check_function_bodies = off;

CREATE
OR REPLACE FUNCTION public.update_usersinfriendsleague_xp()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
BEGIN
UPDATE public.usersinfriendsleague AS ufl
SET xp = NEW.xp FROM public.usersinbetting AS uib
...
begin
select
into status, content result.status, result.content
...
CREATE
OR REPLACE FUNCTION public.delete_old_profile()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
CREATE
OR REPLACE FUNCTION public.delete_storage_object(bucket text, object text, OUT status integer, OUT content text)
 RETURNS record
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
project_url text := '<YOURPROJECTURL>';
  service_role_key
text := '<YOURSERVICEROLEKEY>'; --  full access needed
  url
text := project_url||'/storage/v1/object/'||bucket||'/'||object;
begin
select
...
CREATE
OR REPLACE FUNCTION public.get_random_player(excluded_players integer[], fav_team_id integer DEFAULT NULL::integer, is_fav_team boolean DEFAULT false)
...
NEXT result;
      ...
      excluded_players
:= array_append(excluded_players, result.id);
END LOOP;
END IF;
END;
$function$
...
CREATE
OR REPLACE FUNCTION public.populate_random_players()
 RETURNS text
 LANGUAGE plpgsql
AS $function$
DECLARE
...
CREATE
OR REPLACE FUNCTION public.reset_random_seed()
 RETURNS void
 LANGUAGE plpgsql
AS $function$
BEGIN
...
CREATE
OR REPLACE FUNCTION public.set_random_seed(seed_value double precision)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
BEGIN
